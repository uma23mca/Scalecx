		...
		function wpgmp_show_buynow_notice($pluginheader){
			...
			if( isset($_GET['page']) && !empty($_GET['page']) && strpos( sanitize_key( $_GET['page'] ), 'wpgmp' ) !== false ){
				...
				$pluginheader .= '<div class="flippercode-ui">
						<div class="fc-main"><div class="buy_premium notice notice-success is-dismissible">'.esc_html__('Hassle-free upgrade to pro version & unlock listing, routes,  directions, filtration, drawing & many more features.','wp-google-map-plugin' ).' <a class="buy_now_link" href="https://codecanyon.net/item/advanced-google-maps-plugin-for-wordpress/5211638" target="_blank">'.esc_html__('Upgrade Now','wp-google-map-plugin').'</a> or <a onclick="return false;" class="fc_ignore_notice hide_buy_notice" href="#">'.esc_html('Don\'t show again','wp-google-map-plugin').'</a></div></div></div>';	
				}
			}
					...
					<div class="notice notice-info is-dismissible sample_notice fcdoc-body"   >
						<div class="notice_b">
							...
							<div class="notice_b_inner">
								<h3><?php esc_html_e('WP Google Map Plugin','wp-google-map-plugin'); ?></h3>
